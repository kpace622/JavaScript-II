...
let runnersLargeSizeShirt = [];
  ...
  if (runners.shirt_size === "L") {
    runnersLargeSizeShirt.push(runners)
  }
});
...
const sum = runners.reduce((sum, runner) => {
  return sum + runner.donation;
}, 0)
ticketPriceTotal = sum;
...
// Problem 1
// Create an array listing only the companies involved and how they donated to the event
let companiesDonation = [];
runners.map((name) => {
  companiesDonation.push(name.company_name + " " + name.donation);
})
console.log(companiesDonation);

// Problem 2
// Create an array listing the name of the companies who donated more than 150 for specialized thank you notes.
let largeCompanyDonation = []
runners.filter((runners) => {
  if (runners.donation >= 150) {
    largeCompanyDonation.push(runners.company_name);
  }
})
console.log(largeCompanyDonation);

// Problem 3
// crete an array listing companies names and the email addresses associated with them.
let companiesEmails = [];
runners.map((name) => {
  companiesEmails.push(name.company_name + " " + name.email);
});
console.log(companiesEmails);